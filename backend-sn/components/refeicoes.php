...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($data['data'], $data['levar_refeicao'], $data['almoco'], $data['almoco_mais_cedo'], $data['almoco_mais_tarde'], $data['jantar'], $data['jantar_mais_cedo'], $data['jantar_mais_tarde'])) {
        ...
        // qualquer uma das outras. Como o formulário agora submete cada
        // secção num pedido separado, o conflito também pode vir de uma
        // inscrição já existente desse dia, não só do pedido atual.
        ...
        if ($almocoPedido === 1 || $jantarPedido === 1) {
            ...
            if ($almocoPedido === 1 && $almocoExistente >= 1) {
                ...
                echo json_encode(["status" => "error", "message" => "Já está inscrito noutra opção de almoço para este dia."]);
                exit();
            }
            if ($jantarPedido === 1 && $jantarExistente >= 1) {
                http_response_code(409);
                echo json_encode(["status" => "error", "message" => "Já está inscrito noutra opção de jantar para este dia."]);
                exit();
            }
        }
